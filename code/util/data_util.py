...
from util.general_util import distance, get_closest_grid, paa
...
def read_wifi_fingerprint(paths):
    ...
    #print("before for loop in data_util, fp_features:")
    #print(fp_features)
    #print("fp_locs")
    #print(fp_locs)
...
def read_ble_fingerprint(paths, beacon_list=None, window_size=1000):
    ...
    for path in paths:
        ...
        for line in lines[3:]:
            ...
            filtered_readings = []
            for r in readings:
                address, uuid, major, minor, rssi, distance, txPower = r.split(',')
                #if currT - lastT > window_size:
                all_beacon.add(uuid.lower() + ',' + major + ',' + minor)
                max_r = float(rssi) if float(rssi) > max_r else max_r
    ...
    for i, fp_feature in enumerate(fp_features):
        ...
        for r in fp_feature:
            ...
            if key in beacon_list:
                feature[beacon_list.index(key)] = float(rssi)
        fp_features[i] = feature

    return np.array(fp_features), fp_locs, all_beacon

def read_ble_fingerprint_2(paths, beacon_list=None, window_size=1000):
    fp_features, fp_locs, all_beacon = [], [], set()
    ...
    for path in paths:
        with open(path, 'r') as f:
            ...
            print(path)
            lines = [x.strip() for x in f.readlines()]
        #timestamps = [tuple(map(float, lines[1].split(','))) for p in lines[1]]
        #print("timestamps:")
        #print(timestamps)
        ...
        for line in lines[1:]:
            ...
            try:
                ...
                while currT >= timestamps[idx][0]:
                    idx += 1
                ratio = (currT - timestamps[idx][0]) / (timestamps[idx + 1][0] - timestamps[idx][0])
            except:
                print("error occured!")
                print(idx)
            # print(x,y)
